...
// 直接引用具体函数文件,避免经 index barrel 造成循环导入
import { checkSourceLoaded } from "./sourceTools";
import { getPbfFeatureListSync } from "./layerTool";
...
// 保持 FeatureTypeEnum 从 mapTool 模块导出的历史兼容（定义已迁至 types.ts 打破循环依赖）
export { FeatureTypeEnum };
...
/**
 * 移除 Marker，支持单个 Marker 或 Marker 数组（批量清理）。
 * @param markers - 单个 Marker 或 Marker 数组
 */
export function removeMarkers(markers: minemap.Marker | minemap.Marker[]) {
