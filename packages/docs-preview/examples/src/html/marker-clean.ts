/**
 * 示例：Marker 清理管理（HTML · FE_utils UMD 变体）
 * 核心 API：FE_utils.removeMarkers / removeMarkersOrPopups
 */
import {
  styleHost,
  createMapHost,
...
export default function render(
  ...
  function addMarkers() {
    ...
    status.info(
      `已创建 ${markers.length} 个 Marker（FE_utils.removeMarkers 可批量清理）`
    );
  }

  function cleanMarkers() {
    FE_utils.removeMarkers(markers as any);
    markers.length = 0;
    status.info("FE_utils.removeMarkers(markers) 已执行：全部 Marker 已移除");
  }
  ...
  function addMixed() {
    ...
    status.info(
      "已添加 1 个 Marker + 1 个 Popup（FE_utils.removeMarkersOrPopups 可统一清理）"
    );
  }

  function cleanMixed() {
    FE_utils.removeMarkersOrPopups(mixed);
    mixed.length = 0;
    status.info("FE_utils.removeMarkersOrPopups(mixed) 已执行：Marker + Popup 全部移除");
  }
  ...
  return () => {
    disposed = true;
    if (map) {
      FE_utils.removeMarkersOrPopups([...markers, ...mixed]);
    }
    container.innerHTML = "";
  };
}
