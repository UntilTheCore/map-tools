...
export default function render(
  ...
  function readSync() {
    if (!map) return;
    const features = getPbfFeatureListSync(map, layerId);
    log.log(
  ...
  function readAsync() {
    ...
    checkSourceLoaded({ map, sourceId, limit: 15 })
      .then((loaded) => {
        if (!loaded) {
          log.log("数据源加载超时（token 可能无 Landuse 数据权限）");
          status.error("checkSourceLoaded 超时：请确认 token 具备对应数据权限");
          return;
        }
        log.log("数据源已加载，getPbfFeatureListAsync 读取要素…");
        return getPbfFeatureListAsync(map!, layerId, sourceId, {
          limit: 15,
        }).then((features) => {
