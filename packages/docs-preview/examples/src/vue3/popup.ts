...
import { createPopupDom } from "@ym/map-tools/vue3";
import {
  createLayerId,
  createSourceId,
  ensureLayers,
  ...
  setup(props) {
    return () =>
      h(
        ...
        [
          h("strong", { style: { color: "#4de08b" } }, "Vue 3 Popup"),
          h("div", { style: { marginTop: "4px" } }, `经度: ${props.lngLat.lng.toFixed(4)}`),
          h("div", { style: { marginTop: "2px" } }, `纬度: ${props.lngLat.lat.toFixed(4)}`),
        ],
      );
  },
});
...
export default function render(container: HTMLElement, options: RenderOptions): () => void {
  ...
  createMinemapMap(host, options.token, {}, (currentMap) => {
    currentMap.on("click", (event) => {
      const handle = createPopupDom(PopupContent, {
        lngLat: event.lngLat,
      });
    ...
    .then((currentMap) => {
      ...
      status.info("点击地图创建 Vue 3 Popup");
    })
    .catch((error: unknown) => {
